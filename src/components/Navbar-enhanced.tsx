        ...
        <div className="flex justify-between items-center py-4">
          ...
          <div className="lg:hidden flex items-center space-x-4">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => setDarkMode(!darkMode)}
              className="p-2 bg-white/10 backdrop-blur-lg border border-white/20 rounded-xl text-white"
            >
              {darkMode ? <Sun size={20} /> : <Moon size={20} />}
            </motion.button>
              ...
              {navigation.map((item, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <span className="font-medium">{item.name}</span>
                    {item.badge && (
                      <span className="px-2 py-0.5 bg-gradient-to-r from-green-400 to-blue-500 text-white text-xs font-bold rounded-full">
                        {item.badge}
                      </span>
                    )}
                  </Link>
              ...
              <div className="pt-4 border-t border-white/10 space-y-3">
                <Link to="/voice-bot" onClick={() => setIsOpen(false)}>
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full px-4 py-3 bg-gradient-to-r from-purple-500 to-pink-600 text-white font-medium rounded-xl flex items-center justify-center space-x-2"
                  >
                    <Bot size={16} />
                    <span>Try Voice AI</span>
                  </motion.button>
                ...
                <Link to="/contact" onClick={() => setIsOpen(false)}>
                  <motion.button
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="w-full px-4 py-3 bg-gradient-to-r from-blue-500 to-cyan-600 text-white font-bold rounded-xl flex items-center justify-center space-x-2"
                  >
                    <Sparkles size={16} />
                    <span>Get Started</span>
                  </motion.button>
